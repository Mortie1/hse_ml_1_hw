...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        """
        if self.training:
            # считаем статистики (№1)
            batch_size = input.shape[0]
            inv_bs = np.rec
            self.mean = 1 / batch_size * np.sum(input, axis=0)
            self.input_mean = input - self.mean
            self.var = np.reciprocal(batch_size) * np.sum(self.input_mean * self.input_mean, axis=0)
            self.sqrt_var = np.sqrt(self.var)
            self.inv_sqrt_var = np.reciprocal(self.sqrt_var + self.eps)
            
            # нормирвка (№2)
            norm_input = self.input_mean * self.inv_sqrt_var
            
            if self.affine:
                output = norm_input * self.weight + self.bias
            else:
                output = norm_input
            return output
